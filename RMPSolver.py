...
def display_RMP_solution_dual(depth,prob,CGiter):
    ...
    for i in range(num_features): #constraint (15),
        ...
        for j in range(num_nodes):
            ...
            for l in left_leaves:
                ...
                idx = left_leaves.index(l)

                print("Pi("+str(i)+"_"+str(j)+"_"+str(l)+"-15-)*= "+"%.2f" % round(prob.solution.get_dual_values(start_index+i*num_nodes+j*len(left_leaves)+idx),2))
    ...
    for i in range(num_features): #constraint (16),
        ...
        for j in range(num_nodes):
            ...
            for l in right_leaves:
                ...
                idx = right_leaves.index(l)

                print("Pi("+str(i)+"_"+str(j)+"_"+str(l)+"-16-)*= "+"%.2f" % round(prob.solution.get_dual_values(start_index+i*num_nodes+j*len(right_leaves)+idx),2))
    ...
    for r in range(data_size): #constraint (17),
            ...
            print("Th_"+str(r)+"*= "+"%.2f" % round(prob.solution.get_dual_values(start_index+r),2))
    ...
    for l in range(num_leafs): #constraint (18),
         ...
         print("Be_"+str(l)+"*= "+"%.2f" % round(prob.solution.get_dual_values(start_index+l),2))
    ...
    for r in range(data_size): #constraint (19),
        ...
        for l in range(num_leafs):
            ...
            print("Gm_"+str(r)+"_"+str(l)+"*= "+"%.2f" % round(prob.solution.get_dual_values(start_index+r*num_leafs+l),2))
...
def display_RMP_solution_primal(depth,prob,CGiter,segments_set):
    ...
    for j in range(num_nodes):
        ...
        for i in range(num_features):
            ...
            print("f_"+str(i)+"_"+str(j)+"*= "+"%.2f" % round(prob.solution.get_values(VARS["node_feature_" + str(j) + "_" + str(i)]),2))

    # value used in the boolean test in node n, integer. On the paper: c_{j}

    for j in range(num_nodes):
        
        print("c_"+str(j)+"*= "+"%.2f" % round(prob.solution.get_values(VARS["node_constant_" + str(j)]),2))
    ...
    for l in range(num_leafs):
        ...
        for t in range(get_num_targets()):
            ...
            print("p_"+str(l)+"_"+str(t)+"*= "+"%.2f" % round(prob.solution.get_values(VARS["prediction_type_" + str(t) + "_" + str(l)]),2))
            
    # row error, variables to minimize. On the paper: e_{r}

    for r in range(data_size):
        
        print("e_"+str(r)+"*= "+"%.2f" % round(prob.solution.get_values(VARS["row_error_" + str(r)]),2))
        
    for l in range(num_leafs): # x_{l,s}
    
        for s in range(len(segments_set[l])): 
            
            print("x_"+str(l)+"_"+str(s)+"*= "+"%.2f" % round(prob.solution.get_values(VARS["segment_leaf_" + str(s) + "_" + str(l)]),2))
